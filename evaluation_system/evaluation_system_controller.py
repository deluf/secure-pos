...
class EvaluationSystemController:
    ...
    def __init__(self):
        ...
        try:
            self.local_config = load_and_validate_json_file(self.LOCAL_CONFIG_PATH, self.LOCAL_SCHEMA_PATH)
            print(f"[Evaluation System] Local configuration loaded.")
        ...
        try:
            shared_config = load_and_validate_json_file(self.SHARED_CONFIG_PATH, self.SHARED_SCHEMA_PATH)
            ...
            print(
                f"[Evaluation System] Shared configuration loaded. Port: {self.PORT}, Service Flag: {self.service_flag}")
    ...
    def run(self):
        ...
        """
        print(f"[Evaluation System] System started on port {self.PORT}. Waiting for data streams...")
        ...
        while True:
            ...
            while not self.db.sufficient_labels_query(self.local_config['minNumberLabels']):
                ...
                # Store Prediction immediately. DB handles finding the actual label or creating a new record.
                self.db.store_label(
                ...
                # Store Actual immediately. DB handles finding the prediction or creating a new record.
                self.db.store_label(
